	...
	async.waterfall [
		...
		(docs, next) ->
			...
			while dLength--
				...
				new_doc.vk = if doc.social then doc.social.vk.id else ''
				new_doc.ok = if doc.social then doc.social.ok.id else ''
				new_doc.fb = if doc.social then doc.social.fb.id else ''
