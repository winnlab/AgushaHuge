require "node-zip"
fs = require("fs")
Date::getJulian = ->
  Math.floor (this / 86400000) - (@getTimezoneOffset() / 1440) + 2440587.5

Date::oaDate = ->
  (this - new Date(Date.UTC(1899, 11, 30))) / (24 * 60 * 60 * 1000)

templateXLSX = ""
sheetFront = "<?xml version=\"1.0\" encoding=\"utf-8\"?><x:worksheet xmlns:ss=\"urn:schemas-microsoft-com:office:spreadsheet\" xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\" xmlns:x=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\" xmlns:r=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships\" xmlns:mc=\"http://schemas.openxmlformats.org/markup-compatibility/2006\" mc:Ignorable=\"x14ac\" xmlns:x14ac=\"http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac\"><x:sheetPr><x:outlinePr summaryBelow=\"1\" summaryRight=\"1\" /></x:sheetPr><x:sheetViews><x:sheetView tabSelected=\"0\" workbookViewId=\"0\" /></x:sheetViews><x:sheetFormatPr defaultRowHeight=\"15\" />"
sheetBack = "<x:printOptions horizontalCentered=\"0\" verticalCentered=\"0\" headings=\"0\" gridLines=\"0\" /><x:pageMargins left=\"0.75\" right=\"0.75\" top=\"0.75\" bottom=\"0.5\" header=\"0.5\" footer=\"0.75\" /><x:pageSetup paperSize=\"1\" scale=\"100\" pageOrder=\"downThenOver\" orientation=\"default\" blackAndWhite=\"0\" draft=\"0\" cellComments=\"none\" errors=\"displayed\" /><x:headerFooter /><x:tableParts count=\"0\" /></x:worksheet>"
sharedStringsFront = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><x:sst xmlns:x=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\" uniqueCount=\"$count\" count=\"$count\">"
sharedStringsBack = "</x:sst>"
shareStrings = undefined
convertedShareStrings = undefined
exports.executeAsync = (config, compression, callBack) ->
  process.nextTick ->
    r = exports.execute(config, compression)
    callBack r
    return


exports.execute = (config, compression = 'DEFLATE') ->
  cols = config.cols
  data = config.rows
  colsLength = cols.length
  xlsx = new JSZip(templateXLSX,
    base64: true
    checkCRC32: false
  )
  sheet = xlsx.file("xl/worksheets/sheet.xml")
  sharedStringsXml = xlsx.file("xl/sharedStrings.xml")
  rows = ""
  row = ""
  colsWidth = ""
  styleIndex = undefined
  k = undefined
  if config.stylesXmlFile
    path = config.stylesXmlFile
    styles = null
    styles = fs.readFileSync(path, "utf8")
    xlsx.file "xl/styles.xml", styles  if styles
  shareStrings = new Array()
  convertedShareStrings = ""

  #first row for column caption
  row = "<x:row r=\"1\" spans=\"1:" + colsLength + "\">"
  colStyleIndex = undefined
  k = 0
  while k < colsLength
    colStyleIndex = cols[k].captionStyleIndex or 0
    row += addStringCol(getColumnLetter(k + 1) + 1, cols[k].caption, colStyleIndex)
    colsWidth += "<col customWidth = \"1\" width=\"" + cols[k].width + "\" max = \"" + (k + 1) + "\" min=\"" + (k + 1) + "\"/>"  if cols[k].width
    k++
  row += "</x:row>"
  rows += row

  #fill in data
  i = undefined
  j = undefined
  r = undefined
  cellData = undefined
  currRow = undefined
  cellType = undefined
  dataLength = data.length
  i = 0
  while i < dataLength
    r = data[i]
    currRow = i + 2

    row = "<x:row r=\"" + currRow + "\" spans=\"1:" + colsLength + "\">"
    j = 0
    while j < colsLength
      styleIndex = null
      cellData = r[j]
      cellType = cols[j].type
      if typeof cols[j].beforeCellWrite is "function"
        e =
          rowNum: currRow
          styleIndex: null
          cellType: cellType

        cellData = cols[j].beforeCellWrite(r, cellData, e)
        styleIndex = e.styleIndex or styleIndex
        cellType = e.cellType
      switch cellType
        when "number"
          row += addNumberCol(getColumnLetter(j + 1) + currRow, cellData, styleIndex)
        when "date"
          row += addDateCol(getColumnLetter(j + 1) + currRow, cellData, styleIndex)
        when "bool"
          row += addBoolCol(getColumnLetter(j + 1) + currRow, cellData, styleIndex)
        else
          row += addStringCol(getColumnLetter(j + 1) + currRow, cellData, styleIndex)
      j++
    row += "</x:row>"
    rows += row
    i++
  sheetFront += "<cols>" + colsWidth + "</cols>"  if colsWidth isnt ""
  xlsx.file sheet.name, sheetFront + "<x:sheetData>" + rows + "</x:sheetData>" + sheetBack
  if shareStrings.length > 0
    sharedStringsFront = sharedStringsFront.replace(/\$count/g, shareStrings.length)
    xlsx.file sharedStringsXml.name, (sharedStringsFront + convertedShareStrings + sharedStringsBack)
  results = xlsx.generate(
    base64: false
    compression: compression
  )

  results

startTag = (obj, tagName, closed) ->
  result = "<" + tagName
  p = undefined
  for p of obj
    result += " " + p + "=" + obj[p]
  unless closed
    result += ">"
  else
    result += "/>"
  result

endTag = (tagName) ->
  "</" + tagName + ">"

addNumberCol = (cellRef, value, styleIndex) ->
  styleIndex = styleIndex or 0
  if value is null
    ""
  else
    "<x:c r=\"" + cellRef + "\" s=\"" + styleIndex + "\" t=\"n\"><x:v>" + value + "</x:v></x:c>"

addDateCol = (cellRef, value, styleIndex) ->
  styleIndex = styleIndex or 1
  if value is null
    ""
  else
    "<x:c r=\"" + cellRef + "\" s=\"" + styleIndex + "\" t=\"n\"><x:v>" + value + "</x:v></x:c>"

addBoolCol = (cellRef, value, styleIndex) ->
  styleIndex = styleIndex or 0
  return ""  if value is null
  if value
    value = 1
  else
    value = 0
  "<x:c r=\"" + cellRef + "\" s=\"" + styleIndex + "\" t=\"b\"><x:v>" + value + "</x:v></x:c>"

addStringCol = (cellRef, value, styleIndex) ->
  styleIndex = styleIndex or 0
  return ""  if value is null
  value = value.replace(/&/g, "&amp;").replace(/'/g, "&apos;").replace(/>/g, "&gt;").replace(/</g, "&lt;")  if typeof value is "string"
  i = shareStrings.indexOf(value)
  if i < 0
    i = shareStrings.push(value) - 1
    convertedShareStrings = convertedShareStrings + "<x:si><x:t>" + value + "</x:t></x:si>"
  "<x:c r=\"" + cellRef + "\" s=\"" + styleIndex + "\" t=\"s\"><x:v>" + i + "</x:v></x:c>"

getColumnLetter = (col) ->
  throw "col must be more than 0"  if col <= 0
  array = new Array()
  while col > 0
    remainder = col % 26
    col /= 26
    col = Math.floor(col)
    if remainder is 0
      remainder = 26
      col--
    array.push 64 + remainder
  String.fromCharCode.apply null, array.reverse()
